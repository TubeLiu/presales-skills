...
一个 umbrella marketplace，把售前场景下的 AI 辅助工作流按职责拆成 5 个独立的 Claude Code plugin，通过同一个 marketplace 统一分发。
...
### Step 2：装齐 5 个 plugin

**依赖顺序**：先装共享 plugin（drawio / ai-image），再装主 plugin——否则主 plugin 会因为找不到依赖而运行失败。
...
预期 reload 输出：`6 plugins · XX skills · 8 agents · 1 hook · 4 plugin MCP servers`
- 1 hook：solution-master 的 SessionStart 铁律注入
- 4 MCP：`anythingllm-sm`（solution-master）+ `anythingllm-tw`（tender-workflow）+ 其他 baseline
...
├── .claude-plugin/
│   └── marketplace.json                   # 声明 5 个 plugin 的入口
├── README.md                              # 本文件
│
...
│   ├── .claude-plugin/plugin.json         # 含 mcpServers.anythingllm-sm
...
│   │   ├── writing-skills/
│   │   └── web-access/
│   └── tools/mcp-anythingllm/             # AnythingLLM MCP server（Node）
│
...
└── tender-workflow/                       # 主 plugin：招投标工作流
    ├── .claude-plugin/plugin.json         # 含 mcpServers.anythingllm-tw
    ├── skills/
    │   ├── tpl/SKILL.md                   # /tpl 招标文件生成
    │   ├── taa/SKILL.md                   # /taa 招标分析
    │   ├── taw/SKILL.md                   # /taw 标书撰稿
    │   ├── trv/SKILL.md                   # /trv 审核
    │   └── twc/SKILL.md                   # /twc 配置管理
    └── tools/
        ├── tw_config.py                   # 配置工具（/twc 委托）
        └── mcp-anythingllm/               # AnythingLLM MCP server（Node）
```
...
多个 plugin 内嵌同名 MCP server 会冲突。本仓库采用 **plugin 前缀**：

- solution-master 的 anythingllm MCP → `anythingllm-sm`
- tender-workflow 的 anythingllm MCP → `anythingllm-tw`

安装后会注册两个独立的 MCP server，互不干扰。`allowed-tools` 前缀匹配模式：`mcp__anythingllm-sm__*` / `mcp__anythingllm-tw__*`。

### 7. 版本

当前版本：**v0.1.2**（所有 5 个 plugin 同步）
