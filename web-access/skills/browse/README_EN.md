...
`scripts/mcp_installer.py` registers search-class MCP servers (tavily / exa / minimax-token-plan) into `~/.claude.json`. Called by the setup wizards of [tender-workflow](https://github.com/Alauda-io/skills) and [solution-master](https://github.com/Alauda-io/skills); also runnable standalone via CLI.
