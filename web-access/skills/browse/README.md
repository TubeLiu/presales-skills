...
`scripts/mcp_installer.py` 用于把搜索类 MCP server（tavily / exa / minimax-token-plan）注册到 `~/.claude.json`。被 [tender-workflow](https://github.com/Alauda-io/skills) 与 [solution-master](https://github.com/Alauda-io/skills) 的 setup wizard 调用，也可单独 CLI 使用。
...
> **路径写法**：上面例子用 web-access 自身 skill 加载时设置的 `${CLAUDE_SKILL_DIR}`。被 tender-workflow / solution-master setup wizard 调用时，wizard 用 `installed_plugins.json` bootstrap 探针解析成绝对路径，因此命令形如 `python3 "$WA_INSTALLER" check uv`（`WA_INSTALLER=<web-access install path>/skills/browse/scripts/mcp_installer.py`）。两种写法等价，按调用上下文选。
...
核心是「像人一样思考」的 4 步决策框架：

1. **拿到请求** — 明确目标，定义成功标准
2. **选择起点** — 按任务性质选最可能直达的方式
3. **过程校验** — 每一步结果对照成功标准，发现方向错立即调整
4. **完成判断** — 达成成功标准就停止，不为"完整"过度操作

执行细则（每次联网任务的具体决策点）见 [SKILL.md §浏览哲学](./SKILL.md)。
