...
```bash
python3 tools/tw_config.py get trv default_level
```
...
1. 执行 `python3 tools/docx_encoding_check.py --fix <file_path> --max-retries 3`
...
1. **读取子 Agent 提示词模板**：
   - 使用 Read 工具读取 `.claude/skills/trv/prompts/fullbid_chapter_agent.yaml`
...
2. **读取一致性检查模板**：
   - 使用 Read 工具读取 `.claude/skills/trv/prompts/fullbid_consistency_agent.yaml`
...
1. **加载检查清单**：
   - 使用 Read 工具读取 `.claude/skills/trv/prompts/completeness.yaml`
   - 根据 --type 参数选择对应的检查清单
...
1. **加载审查要点**：
   - 使用 Read 工具读取 `.claude/skills/trv/prompts/compliance.yaml`
   - 根据 --type 参数选择对应的审查要点
...
1. **加载分析模板**：
   - 使用 Read 工具读取 `.claude/skills/trv/prompts/scoring_alignment.yaml`
   - 根据 --type 参数选择对应的分析模板
...
1. **加载风险清单**：
   - 使用 Read 工具读取 `.claude/skills/trv/prompts/risk_check.yaml`
...
- 完整性检查清单: .claude/skills/trv/prompts/completeness.yaml
- 合规性审查要点: .claude/skills/trv/prompts/compliance.yaml
- 评分契合度分析: .claude/skills/trv/prompts/scoring_alignment.yaml
- 风险识别清单: .claude/skills/trv/prompts/risk_check.yaml
...
4. **修订后编码检查**：
   - 若修订成功，继续执行：
     ```bash
     python3 tools/docx_encoding_check.py --fix <revised_docx> --max-retries 3
     ```
