...
2. 将输出的 Markdown 表格直接展示给用户（固定格式，数据来自 `${CLAUDE_PLUGIN_ROOT}/../ai-image/prompts/ai_image_models.yaml`）
3. 表格中标记当前配置的默认模型（`←`）
4. 底部显示如何更改默认模型的命令提示
...
1. 读取当前注册表 `${CLAUDE_PLUGIN_ROOT}/../ai-image/prompts/ai_image_models.yaml`，记录现有模型列表
...
5. 询问用户确认是否应用变更
6. 用户确认后：
   a. 更新 `${CLAUDE_PLUGIN_ROOT}/../ai-image/prompts/ai_image_models.yaml`（保持现有格式不变）
