...
3. 若未安装：
   - 提示用户：drawio plugin 未安装，请在 Claude Code 里执行：
     ...
     ```
   - 若 marketplace `presales-skills` 尚未注册，先 `/plugin marketplace add Alauda-io/skills`（远程订阅）或本地路径 `/plugin marketplace add /path/to/presales-skills`
...
> ```
> 然后让用户在全 tool 清单里人工挑（FQN 仍按 `mcp__<server>__<tool>` 规则；非搜索类 tool 用户不应选，但允许）。
...
特殊场景：
- 只有 1 项（如只有 WebSearch）→ 不问，直接选它
- 0 项（解析失败 / 完全没结果）→ 提示 "没检测到任何 MCP 搜索工具，建议跑 `配置 web-access` 注册 tavily/exa/minimax，或继续用 WebSearch 兜底"，把 priority 设成 `["WebSearch"]` 进 4.5

**c. AskUserQuestion 让用户选默认**（单选 + "全用 + 按编号顺序降级"选项）

> "选 taw / taa 跑 web 检索时的默认搜索工具：
...
```bash
# 例 1：用户选了 minimax
python3 $SKILL_DIR/tools/tw_config.py set mcp_search.priority '["mcp__minimax__web_search"]'

# 例 2：用户选了"全用"，按编号顺序排
python3 $SKILL_DIR/tools/tw_config.py set mcp_search.priority \
  '["mcp__tavily__tavily_search", "mcp__exa__web_search_exa", "mcp__minimax__web_search", "WebSearch"]'

# 例 3：0 项兜底
python3 $SKILL_DIR/tools/tw_config.py set mcp_search.priority '["WebSearch"]'
```

> **AI 注**：`set` 命令收到老别名（如 `tavily_search`）会自动转 FQN 落盘——但**新写的 priority 必须用 FQN**，不要再造别名。落盘后 `python3 $SKILL_DIR/tools/tw_config.py get taw mcp_search.priority` 查 confirm。

**e. 显示当前 priority + 提示**

```
✅ 已写入 mcp_search.priority = ["mcp__minimax__web_search"]
   工作流（taw / taa）跑 web 检索时按列表顺序试，全失败回 WebSearch（永远兜底）。
   如果未来新装其它 MCP 想加进来，重跑 /twc setup 这一步即可。
```
