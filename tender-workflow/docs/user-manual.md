...
本项目的 Skill 文件直接存放在 `skills/` 目录下（项目内），Claude Code 在项目目录中启动时会自动识别，**无需创建软链接**。
...
python3 skills/taw/tools/kb_indexer.py --scan
...
**解决方案**：
```bash
# Skill 存放在项目的 skills/ 下（非软链接），Claude Code 在项目目录中启动时会自动识别
