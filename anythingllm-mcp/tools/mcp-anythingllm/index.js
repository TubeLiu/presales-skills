...
// Read each plugin's `anythingllm:` block as fallback to env vars.
// Priority order (higher overrides lower):
//   1. env var (highest)
//   2. ai-image plugin's config (~/.config/presales-skills/config.yaml)
//   3. tender-workflow's config (~/.config/tender-workflow/config.yaml)
//   4. solution-master's config (~/.config/solution-master/config.yaml)
//   5. defaults (lowest)
// Implementation iterates CONFIG_CANDIDATES in reverse so later (higher-priority) sources overwrite.
// Each plugin holds its own anythingllm settings; this reader merges them so MCP server can pick up
// whichever plugin's settings the user filled in.
