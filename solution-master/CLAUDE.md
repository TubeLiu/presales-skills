...
- 执行 `hooks/session-start`，读取 `skills/solution-master/SKILL.md` 全文并作为 `additionalContext` 注入到上下文
- **项目门禁**（4 条任一满足即触发）：当前 cwd 含以下任一时触发，全局安装下不会污染非 SM 项目：
  1. `drafts/` 目录
  2. `docs/specs/` 目录
  3. `skills/solution-master/SKILL.md`（plugin 模式 / 源码 clone 模式）
...
详细配置工具：`skills/solution-master/scripts/sm_config.py`（solution-master 自家的配置读取/迁移辅助）
