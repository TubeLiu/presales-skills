...
- 上下文感知配图（通过 ai-image plugin 的 `image_gen.py` 与 drawio plugin auto-trigger 调用（自然语言"画图"或 `/drawio:draw`，由 Claude 直接生成 XML，无 Python helper））
...
```
/plugin marketplace add Alauda-io/skills     # 或本地路径 /path/to/presales-skills
...
```bash
npx skills add Alauda-io/skills -a <agent>   # vercel-labs/skills CLI
```
...
- 执行 `hooks/session-start`，读取 `skills/go/SKILL.md` 全文并作为 `additionalContext` 注入到上下文
- **项目门禁**（4 条任一满足即触发）：当前 cwd 含以下任一时触发，全局安装下不会污染非 SM 项目：
  1. `drafts/` 目录
  2. `docs/specs/` 目录
  3. `skills/go/SKILL.md`（plugin 模式 / 源码 clone 模式）
...
详细配置工具：`skills/go/scripts/sm_config.py`（solution-master 自家的配置读取/迁移辅助）
