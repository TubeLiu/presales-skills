...
- 上下文感知配图（通过 ai-image plugin 的 `image-gen` 命令调用）
...
│   # (image-generation 已抽为独立 plugin `ai-image`，位于 presales-skills marketplace 的 ../ai-image/；通过 `image-gen` bin 命令跨 plugin 调用)
├── agents/                           # 子智能体角色定义
│   ├── writer.md
│   ├── spec-reviewer.md
│   └── quality-reviewer.md
...
开始 → solution-brainstorming（苏格拉底式提问）
     ...
     → 对每个章节任务（独立子智能体，subagent-driven-writing 流程）：
         ...
         → ai-image plugin 的 `image-gen` 命令（如需配图）
...
| ai-image (image-gen) | 章节需要配图（由独立 plugin `ai-image` 提供，需配套安装） |
