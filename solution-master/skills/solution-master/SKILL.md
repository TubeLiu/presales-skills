...
详见 `$SKILL_DIR/workflow/config.md`。基本结构：

```yaml
localkb:
  path: /path/to/Local-KnowledgeBase
api_keys:
  ark: <key>          # 火山方舟（Seedream）
  dashscope: <key>    # 阿里云（通义万相）
...
未配置时，AI 图片功能降级为占位符，其他功能正常。配置管理统一通过 ai-image plugin 的 `ai_image_config.py setup|set|show|validate|migrate` 子命令（auto-migrate 会自动合并旧路径）。
