...
**一个让 Claude Code 写出真正能交付的方案文档的插件。**

> 🙏 **致谢上游**：本 plugin 的 SessionStart hook 注入机制、子智能体两阶段审查框架（writer / spec-reviewer / quality-reviewer 三角色 + "不要信任报告"原则）、`<HARD-GATE>` 反合理化标签、铁律注入文本格式等核心反脱轨机制，vendored from [jnMetaCode/superpowers-zh](https://github.com/jnMetaCode/superpowers-zh)（MIT License © 2026 jnMetaCode；上游英文原版 [obra/superpowers](https://github.com/obra/superpowers) © Jesse Vincent）。本仓做了 superpowers-zh → solution-master 的体例适配（角色名 `implementer` → `writer`、`code-reviewer` → `quality-reviewer`、措辞从代码开发调整为文档撰写、注入文本改为 Solution Master 上下文）。感谢上游作者。详细借用文件清单、相对 upstream 的改写说明以及原项目 LICENSE 全文见 [THIRD_PARTY_NOTICES.md](./THIRD_PARTY_NOTICES.md)。 Solution Master 不是"再来一个 prompt 模板"——它把苏格拉底式需求提取、子智能体隔离撰写、双阶段独立审查、四路知识融合与企业级 DOCX 排版组装成一条可追溯的流水线，让每一份方案都经过"写—审内容—审质量"的闭环，而不是一次性"写完就交"。
