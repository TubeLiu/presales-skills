...
ppt-master is now distributed as a member plugin of the `presales-skills` umbrella marketplace:

```
/plugin marketplace add Alauda-io/presales-skills     # or local path /path/to/presales-skills
/plugin install ppt-master@presales-skills
```

Once installed, the embedded SKILL automatically responds to triggers like "generate PPT" / "make presentation" / "生成PPT", and invokes internal scripts via `${CLAUDE_SKILL_DIR}/scripts/...`. In plugin mode you do **NOT** need to `cd` into this project or run the `python3` commands shown below — Claude invokes them for you.

The "Quick Start" section below describes the **source-mode** workflow (clone this repo, run scripts manually). Both modes coexist.
