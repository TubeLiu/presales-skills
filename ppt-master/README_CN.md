...
ppt-master 现作为 `presales-skills` umbrella marketplace 的成员 plugin 分发：
...
安装后 plugin 内置的 SKILL 会自动响应"生成 PPT"、"做演示文稿"等触发语，并用 `${CLAUDE_SKILL_DIR}/scripts/...` 调用内部脚本——**plugin 模式下你不需要手动 cd 进项目或执行下文的 `python3` 命令**，Claude 会在合适时机代你调起。

下文"快速开始"段落针对**源码模式**（直接 clone 本仓库，手工运行脚本做 PPT 生成）提供完整流程，两种模式并存。
