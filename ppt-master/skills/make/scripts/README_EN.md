...
**Cross-agent / direct script**: first resolve `$AI_IMAGE_SKILL_DIR` per ai-image SKILL.md §path-self-locate:
```bash
...
Dependency verification (**AI: do NOT hand-write `python3 -c "import X; import Y"`** to guess package names — `requirements.txt` is the authoritative source):

```bash
# The single canonical entry: run _ensure_deps.py, which checks requirements.txt and pip-installs as needed
python3 scripts/_ensure_deps.py
```

> Historical lesson: `cssutils` / `cairosvg` are **not listed in requirements.txt** (the former is unused; the latter is a better drop-in for `svglib` but is only needed when SVG contains complex gradients / filters). Hand-written import checks falsely report "dependency missing" because of stale AI training data.
